...
public class ListImageLoader {
    ...
    public void loadRange(int start, int end, Context context, boolean force) {
        ...
        try {
            ...
            if (Math.abs(end - start) > 30) try {
                throw new Exception("range: " + start + " - " + end);
            } catch (Exception x) {
                Log.w("appkit", x);
            }
            ...
            for (int i = 0; i < cnt; i++) {
                ...
                if (url.equals(loadedUrls.get(makeIndex(item, i))) || force) {
                    if (ImageCache.getInstance(context).isInTopCache(url)) { // (in)sanity check
                        if (DEBUG)
                            Log.v(TAG, "Image [" + item + "/" + i + "] already loaded; skipping");
    ...
    public synchronized void commitPartialCancel() {
        ...
        if (DEBUG)
            Log.w(TAG, "Partial cancellation completed, canceled " + pendingPartialCancel.size() + " tasks");
    ...
    /*package*/
    synchronized void onCacheEntryRemoved(String key) {
        int index;
        while ((index = loadedUrls.indexOfValue(key)) >= 0) {
            long x = loadedUrls.keyAt(index);
            loadedUrls.removeAt(index);
            if (DEBUG)
                Log.i(TAG, "Removed cache entry for [" + getPosition(x) + "/" + getImage(x) + "] " + key);
        }
    }
    ...
    private class RunnableTask implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                if (url.startsWith("A")) { // [A]lternative
                    String[] parts = url.split("\\|");
