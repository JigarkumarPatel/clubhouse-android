...
public class ListImageLoaderWrapper implements AbsListView.OnScrollListener {
    ...
    private void doUpdateImages() {
        ...
        if (viCount <= 0)
            viCount = Math.max(5, list.getLastVisiblePosition() - list.getFirstVisiblePosition());
    ...
    private void realScrollStateChanged(int scrollState) {
        ...
        if (scrollState == SCROLL_STATE_TOUCH_SCROLL && listener != null)
            listener.onScrollStarted();
		/*if(scrollState==SCROLL_STATE_TOUCH_SCROLL){
			imgLoader.cancelAll();
		}*/
        if (scrollListener != null)
            scrollListener.onScrollStateChanged(list.getView() instanceof AbsListView ? ((AbsListView) list.getView()) : null, scrollState);
